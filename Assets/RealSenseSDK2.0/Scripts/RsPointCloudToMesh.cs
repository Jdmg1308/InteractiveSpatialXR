...
{
    private const float BOX_DIST_THRESHOLD = 0.2f;
    private const float PLANE_DIST_THRESHOLD = 0.1f;
...
// // reformat the code so you use vect3
    ...
    public bool usePointDecider(Vector3 point, ref int boxToUpdateKey) {
        ...
        if (minDist <= BOX_DIST_THRESHOLD && DistanceToPlane(minBox, point) < PLANE_DIST_THRESHOLD)
        {
    ...
    private void UpdateSet(int key, Vector3 p)
    {
        ...
        if (p.x < min.x || p.x > max.x || p.y < min.y || p.y > max.y || p.z < min.z || p.z > max.z)
        {
            ...
            if (newBox[6].z - newBox[0].z > 0.5)
            {
                // RotationMethod(key, newBox, GetGameObjectReference(key));
            }
            else
            {
                UpdateMesh(newBox, GetGameObjectReference(key));
                VerticesReference[key] = newBox;
                UpdatePlane(key, Quaternion.identity);
            }
        }
    }

    private void RotationMethod(int key, Vector3[] points, GameObject obj)
    {
        Vector3 minPoint = points[0];
        Vector3 maxPoint = points[0];
    ...
    private void CreateGameObjs(int key, Vector3[] points)
    {
        ...
        newGameObj.GetComponent<MeshRenderer>().material.color = Color.cyan;
    }
